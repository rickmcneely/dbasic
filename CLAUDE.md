...
13. **Go-build error messages reference your `.dbas` source.** Codegen emits `//line` directives, so `cannot use X as Y` and similar Go errors point to the original `.dbas:line`, not a temp `main.go`. Line numbers are exact, including for tokens that end a line (a bare `RETURN`, `CONTINUE`, `WEND`, `ENDIF`); before the lexer fix in `readChar` those were reported one line too far down. Note identifiers still report the *column* just past their last character.
