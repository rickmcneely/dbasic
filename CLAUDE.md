...
13. **Go-build error messages reference your `.dbas` source.** Codegen emits `//line` directives, so `cannot use X as Y` and similar Go errors point to the original `.dbas:line`, not a temp `main.go`. Line and column numbers are both exact: line numbers hold for tokens that end a line (a bare `RETURN`, `CONTINUE`, `WEND`, `ENDIF`), and every token — names, numbers and strings included — reports the column it *starts* at, so the error caret lands on the token rather than just past it.
...
15. **23 runtime functions are now callable**, having existed in `pkg/runtime` but never been registered: `Split`, `Join`, `Reverse`, `InstrRev`, `Hex`/`Oct`/`Bin`, `ArrayLen`, `Input`/`InputInt`/`InputFloat`, `DirExists`, `CopyFile`, `ListDir`, `GetCwd`/`SetCwd`, `BaseName`/`DirName`/`JoinPath`, `Environ`/`SetEnviron`, `GetArgs`, `Exit`, `Weekday`. The ones that can fail return `ERROR` so they work with ONERR. Two notes: (a) **a program's own function shadows a built-in of the same name** — the analyzer allows the redefinition and codegen suppresses its helper, so adding built-ins cannot break existing code (DBtui's own `joinPath`/`baseName`/`dirName` would otherwise have broken); (b) `Input` and `Exit` double as statement keywords and are told apart by the bracket — `INPUT name` vs `Input("? ")`, `EXIT FOR` vs `Exit(1)`. All input paths (`INPUT`, `LINE INPUT`, `Input()`) now share one reader; they used to make one each, and mixing them silently skipped lines.

16. **Builtins are checked against the code generator by a test.** Every name the analyzer accepts must have an entry in `runtimeFuncDefs` (or be listed in `emittedInline` in `TestEveryBuiltinCanBeEmitted` if codegen rewrites it, as it does for `NewError`/`Errorf`/`Printf`/`Sprintf`), and any helper using a package must list it in `runtimeFuncImports`. The two lists had drifted badly — 36 builtins including `Randomize`, `Now`, `Date`, `Replace` and `PI` passed `dbasic check` and then failed `go build` with "not defined", and `Str` alone in a program failed because its `fmt` import was undeclared. Adding a builtin means adding both halves; the tests in `pkg/codegen/codegen_test.go` will tell you if you forget.

17. **`dbasic emit` output is deterministic.** The same `.dbas` always produces byte-identical Go, so `emit` output can be diffed or used as a golden file. Getting there meant sorting the two maps that have no natural order (imports — which also matches gofmt — and injected runtime helpers) and recording source order for the two that do (`StructLiteral.Order`, `JSONLiteral.Order`), since struct and JSON literal fields are stored in maps. Emit struct/JSON literal fields via `literalKeyOrder`, never by ranging the map: Go randomises map iteration and the generated code would change on every build.

18. **`dbasic-lsp` provides editor language support.** The LSP binary at `cmd/dbasic-lsp` runs over stdio and publishes: parse + analyze diagnostics; document symbols (file outline); hover (signature for top-level identifiers); go-to-definition; completions (keywords + top-level idents, member completions after `.`); find-references; rename. Wire it up in any LSP-aware editor (VS Code, Neovim, Helix, etc.). Note: rename and find-references are document-wide and do not honor scope — a local variable and a top-level symbol with the same name are treated as the same. Workable for top-level symbols (subs, functions, types, constants).

19. **`STEP`/`BYTES`/`STRING`/`TYPE` may now be used as identifiers** in declaration positions (DIM/LET/CONST/parameter/field/sub/function/method names) AND in expression contexts. The parser disambiguates statement-start `TYPE` based on what follows (`TYPE Foo` is still a type-decl; `TYPE = 5` and `TYPE.X()` are assignments/calls on a variable named TYPE).

20. **Explicit Go generic instantiation:** use `pkg.Func(OF T1, T2)(args)` when type inference can't resolve the type parameters. Example: `slices.Sort(OF []INTEGER)(nums)` emits `slices.Sort[[]int](nums)`.

21. **`dbasic fmt` is a token-stream formatter.** Comment-preserving, idempotent. Default prints to stdout; `-w` writes back; `-l` lists files whose formatting differs from the formatter's output.
...
Games all live under `examples/games/`: `bombsquad/` (BombSquad — a Minesweeper-style game; Ebitengine, mouse/menus/dialogs/high scores/sound), `gagala/` (Gagala — a Galaga-style arcade shooter; Ebitengine, flight paths/formation/tractor beam), `pacman/` (Ebitengine game with audio/multiplayer), `gorillas/` (terminal game), `tictactoe/` (web server).
